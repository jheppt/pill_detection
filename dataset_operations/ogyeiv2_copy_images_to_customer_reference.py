...
def main():
    ...
    dst_imgs_customer = dataset_images_path_selector("ogyeiv2").get("customer").get("customer_images")
    dst_masks_customer = dataset_images_path_selector("ogyeiv2").get("customer").get("customer_mask_images")
    dst_labels_customer = dataset_images_path_selector("ogyeiv2").get("customer").get("customer_segmentation_labels")

    dst_imgs_reference = dataset_images_path_selector("ogyeiv2").get("reference").get("reference_images")
    dst_masks_reference = dataset_images_path_selector("ogyeiv2").get("reference").get("reference_mask_images")
    dst_labels_reference = dataset_images_path_selector("ogyeiv2").get("reference").get("reference_segmentation_labels")
    ...
    for f in src_img_files:
        # Get the filename without the extension and the last part of the path the 5 last characters lactiv_plus_u_014.jpg -> lactiv_plus_
        filename = os.path.basename(f).split(".")[-2][:-6]
